...
const entityConfig = {
    ...
    farmers: {
        endpoint: "/api/Agricultores",
        title: "Agricultores",
        singular: "agricultor",
        idField: "idAgricultor",
        kicker: "Produtores rurais",
        description: "Cadastre agricultores vinculando associacao e cultivo acompanhado."
    },
    crops: {
...
function cacheElements() {
    [
        ...
        "refreshHistory", "regionSelect", "diagnosisCropSelect", "consultClimate", "generateDiagnosis",
...
function bindEvents() {
    ...
    elements.diagnosisCropSelect.addEventListener("change", handleDiagnosisCropChange);
...
async function loadDashboard() {
    try {
        ...
        setText("totalAgricultores", data.totalAgricultores);
...
function populateDiagnosisCropSelect() {
    if (!elements.diagnosisCropSelect) {
        return;
    }

    if (!state.crops.length) {
        elements.diagnosisCropSelect.innerHTML = '<option value="">Nenhum cultivo cadastrado</option>';
        elements.diagnosisCropSelect.disabled = true;
        updateDiagnosisButtonState();
        return;
    }

    const currentValue = elements.diagnosisCropSelect.value;
    elements.diagnosisCropSelect.innerHTML = state.crops.map((crop) =>
        `<option value="${crop.idCultivo}">${escapeHtml(crop.nomeCultivo)}</option>`
    ).join("");

    const stillExists = state.crops.some((crop) => String(crop.idCultivo) === String(currentValue));
    elements.diagnosisCropSelect.value = stillExists ? currentValue : String(state.crops[0].idCultivo);
    elements.diagnosisCropSelect.disabled = false;
    updateDiagnosisButtonState();
}

function handleDiagnosisCropChange() {
    elements.diagnosisResult.classList.add("d-none");
    updateDiagnosisButtonState();
}

function updateDiagnosisButtonState() {
    elements.generateDiagnosis.disabled = !state.selectedRegionId || !elements.diagnosisCropSelect?.value;
}
...
async function generateDiagnosis() {
    ...
    const selectedCropId = elements.diagnosisCropSelect?.value;

    if (!selectedCropId) {
        showToast("Selecione um cultivo antes de gerar o diagnostico.", "warning");
        return;
    }
    ...
    try {
        const data = await apiFetch(`/api/Diagnostico/regiao/${state.selectedRegionId}/cultivo/${selectedCropId}`);
...
function renderDetailedDiagnosis(data) {
    ...
    setText("diagnosisCrop", data.cultivo || findCropName(data.idCultivo || elements.diagnosisCropSelect?.value));
...
function getManagementTableDefinition(entity) {
    const definitions = {
        ...
        farmers: {
            headers: ["Nome", "CPF", "Telefone", "Associacao", "Cultivo"],
            cells: (record) => [
                `<strong>${escapeHtml(record.nomeAgricultor)}</strong>`,
                escapeHtml(record.cpf),
                escapeHtml(record.telefone),
...
function openEntityForm(record = null) {
    ...
    if (
        ...
        showToast("Cadastre uma associacao e um cultivo antes do agricultor.", "warning");
        return;
    }
...
function buildEntityForm(entity, record = {}) {
    ...
    if (entity === "farmers") {
        return `
            ${formInput("nomeAgricultor", "Nome do agricultor", record.nomeAgricultor, "text", 6, 'maxlength="80" placeholder="Jose Ferreira da Silva"')}
            ${formInput("cpf", "CPF", record.cpf, "text", 6, 'maxlength="11" minlength="11" inputmode="numeric" placeholder="12345678901"')}
            ${formInput("telefone", "Telefone", record.telefone, "text", 6, 'maxlength="15" minlength="10" inputmode="numeric" placeholder="81999998888"')}
            ...
            <div class="col-12"><div class="form-text">O cadastro retorna automaticamente a associacao e o cultivo do agricultor.</div></div>`;
    }
...
function collectEntityPayload(entity, formData) {
    ...
    if (entity === "farmers") {
        return {
            nomeAgricultor: value("nomeAgricultor"),
...
function getRecordLabel(entity, record) {
    const labels = {
        ...
        farmers: `${record.nomeAgricultor} - ${record.nomeAssociacao || findAssociationName(record.idAssociacao)}`,
...
function setClimateLoading(isLoading) {
    ...
    elements.generateDiagnosis.disabled = isLoading || !state.selectedRegionId || !elements.diagnosisCropSelect?.value;
